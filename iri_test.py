...
def frontend_main():
    ...
    if uploaded_file is not None:
        ...
        df_input_copy.replace(['NA', 'N/A', 'n/a', 'null', 'NaN', 'NAN'], np.nan,
                              inplace=True)

        inputDF_Nulls = df_input_copy.isna()
    ...
    if imputation_started:
        ...
        st.image('./item_recommender/iri_metrics_legend.png', caption='Color coding key')
        ...
        df_accuracy_metrics_as_percents = df_accuracy_metrics_as_percents.applymap(lambda
                                                                                       x: f'{int(x)}%' if x != 0 else '--')  # Don't display untouched data as zero to avoid confusion
        with st.expander("**Model Accuracy Data**"):
            st.dataframe(df_accuracy_metrics_as_percents, column_config={
                "Item Code": st.column_config.TextColumn(),
                "UPCItem": st.column_config.TextColumn(),
                "UPCCase": st.column_config.TextColumn(),
                "ADV_ItemUPC": st.column_config.TextColumn(),
